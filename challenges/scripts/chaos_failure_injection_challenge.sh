...
set -uo pipefail
HEALTH_URL="${VISIONENGINE_HEALTH_URL:-}"
CHAOS_HOST="${VISIONENGINE_CHAOS_HOST:-localhost}"
CHAOS_PORT="${VISIONENGINE_CHAOS_PORT:-}"
...
if [[ -z "$HEALTH_URL" ]] || [[ -z "$CHAOS_PORT" ]]; then
    echo "[1/6] SKIP: VISIONENGINE_HEALTH_URL/_PORT unset — SKIP-OK: #env-no-target"
