...
echo "=== VisionEngine Stress Sustained-Load Challenge ==="
echo "  url=$HEALTH_URL dur=${DURATION}s rps=${RPS}"

if [[ -z "$HEALTH_URL" ]]; then
    echo "[1/6] SKIP: VISIONENGINE_VD_HEALTH_URL unset — SKIP-OK: #env-no-target"
    echo "=== VisionEngine Stress Challenge: PASSED (SKIP-OK) ==="
    exit 0
fi
...
echo
echo "=== VisionEngine Stress Challenge: PASSED ==="
