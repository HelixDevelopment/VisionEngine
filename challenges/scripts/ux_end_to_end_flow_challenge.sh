...
echo "=== VisionEngine UX End-to-End Flow Challenge ==="
echo "  bin=$BIN_PATH timeout=${TIMEOUT_SEC}s"

if [[ -z "$BIN_PATH" ]] || [[ ! -x "$BIN_PATH" ]]; then
    echo "[1/5] SKIP: VISIONENGINE_VD_BIN unset — SKIP-OK: #env-binary-missing"
    echo "=== VisionEngine UX Challenge: PASSED (SKIP-OK) ==="
    exit 0
fi
...
echo
echo "=== VisionEngine UX Challenge: PASSED ==="
